...
export function renderListadoNecesidadesPage(params: {
  ...
  return renderPageShell({
    title: "Necesidades abiertas | Manos",
    activePath: "/",
    styles: `${BANDA_PULSO_CSS}${LISTADO_NECESIDADES_CSS}`,
    body: `<main class="board-shell">
    <header class="board-header">
      ...
      <form class="filters" method="get" action="/">
        <label>Estado<select name="estado"><option value="">Abiertas</option>${renderEstados()}</select></label>
...
function renderCard(necesidad: Necesidad, now: Date): string {
  ...
  return `<article class="need-card urgency-${necesidad.urgencia.toLowerCase()}">
    <div class="meta"><span>${necesidad.urgencia}</span><span>${necesidad.estado}</span><span>${necesidad.habilidad.replaceAll("_", " ")}</span><span>${antiguedadMin} min</span></div>
...
function renderEstados(): string {
  return Object.values(EstadoNecesidad)
    .filter((estado) => estado !== EstadoNecesidad.Abierta)
